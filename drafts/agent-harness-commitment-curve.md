...
The first agent is **chorus-agent**: a LangGraph ReAct agent running Qwen3-30B via the SS&C AI Gateway. Its job is narrow and high-stakes — read AWD CSD binary forms and produce Chorus Classic XML, with cross-form analysis and a structured `FormAnalysis` + `CrossFormReport` output contract. The constraint set is not subtle. Multi-agent, regulated financial-services domain, a paid enterprise inference gateway, auditable output, and a hard distinction between Chorus system fields and client LOB fields that has to be preserved or the whole thing is useless. Errors are not tolerable. Hallucinated field names are not tolerable. A cross-form inconsistency that nobody catches is not tolerable. That's the axiom set.

The second agent is **kindle-pipeline**: a script-driven pipeline that pulls books, essays, and web articles from various sources (Marker-OCR'd books, web scrapes from Karakeep-bookmarked essays), produces a Kindle Scribe-targeted PDF with annotation margins plus an Obsidian markdown export, and runs an LLM proofreader over the OCR'd text to catch joined words, LaTeX artifacts, and other Marker-produced weirdness before it commits. It runs locally on Ollama against qwen3:8b on a laptop. Single user — me. No SLA. No compliance regime. No audit trail anyone will ever read. The constraint set is almost exactly the inverse of chorus-agent. The cost of a bad run is that I reread a paragraph. That's the axiom set.

I have applied the same twelve-component taxonomy to both.

## Chorus-agent — where the full twelve was earned
...
**State management.** Analyses span many forms. A single CSD bundle can hold dozens. If the gateway drops the connection halfway through a cross-form compare, recovering from the last good checkpoint is not a nice-to-have — it's the difference between a tolerable failure and a day of lost work. Checkpointing at the per-form boundary mapped cleanly onto the natural seams in the data, which made it cheap to implement and obvious to justify. In regulated work the cost of losing state is real. State management earns its keep.

**Error handling.** The AI Gateway sits behind enterprise networking, enterprise auth, enterprise DNS, enterprise TLS inspection, and the occasional enterprise proxy. Retry with backoff is the minimum; what matters more is distinguishing "I should retry" from "I should surface this to the user because the gateway genuinely rejected the request" from "the model returned malformed output." Three very different failure modes, three very different responses. If you collapse them into "retry harder," you create silent drift in outputs, which in a regulated context is the worst of the possible failures.

**Guardrails.** Injection filtering is non-negotiable when your agent is looking at documents authored by clients. A client could, intentionally or accidentally, include text in a form that reads like an instruction to the model. Agents that do not have at least a bounding envelope around "what inputs from the document can influence behavior" are shipping a security hole. Guardrails in chorus-agent are less about the fanciest prompt-injection defenses and more about the structural fact that the model has a narrow job (analyze the form) and should not be persuaded, by any content whatsoever, to expand that job. Length caps, strict output schemas, a refusal to treat document content as instructions.

**Verification loops.** The cross-form report is a synthesis across independent per-form analyses. The synthesis is exactly the kind of step where LLMs are prone to confabulating consistency. A separate verification pass — check that each claim in the synthesis is grounded in at least one form-level analysis — catches the kind of failure a single-pass agent will never catch about itself. This is not paranoia; it is a structural feature of how generation works. Verification is how you turn a text generator into a reporting system.

**Subagent orchestration.** Per-form analyses are independent and parallelizable. Cross-form synthesis depends on them. This is the textbook case for a parallel-then-sequential pattern. I don't give chorus-agent credit for being clever here — the shape of the data demanded it.
...
*Chris Moore — [draft]. Citation: Alex Ker, "Harnesses are everything. Here's how to optimize yours." https://x.com/thealexker/status/2045203785304232162.*
